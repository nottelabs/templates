...
Template path: `notte-templates/ts/ecommerce-price-monitoring/books-to-scrape-price-tracker`
Runtime: TypeScript, entrypoint `main.ts`.

## Run a price check

```bash
cd notte-templates/ts/ecommerce-price-monitoring/books-to-scrape-price-tracker
cp .env.example .env
...
- Uses a Notte-hosted browser session with deterministic navigation.
- Uses Zod schemas for listing and detail-page structured extraction.
- Defaults to the public demo site at `http://books.toscrape.com/`.
- Opens a Notte browser session and prints the session ID plus viewer link.
...
- Optionally opens each product page for UPC, category, exact availability, and description.
...
- The source exploration found `http://books.toscrape.com/` more reliable than HTTPS.
